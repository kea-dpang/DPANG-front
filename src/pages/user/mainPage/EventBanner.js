...
import bannerImgList from "../../../assets/datas/ProductEventDetailData";
...
function EventSlider() {
  ...
  const [selectEvent, setSelectEvent] = useState(bannerImgList[slideIndex].eventBannerUrL);   // 선택된 이미지 저장 변수
  ...
  const moveToSlide = (direction) => {
    setSlideIndex(prev => 
      direction === "prev" // 왼쪽으로 가는 버튼이었다면
        ? prev === 0 ? bannerImgList.length - 1 : prev - 1 // 왼쪽으로 가는데, 이때 젤 첫 번째 index였다면 젤 끝 index로 가기
        : prev === bannerImgList.length - 1 ? 0 : prev + 1 // 오른쪽으로 가는 버튼이면 오른쪽으로 가는데, 젤 마지막 index 였다면 시작 index로 가기
    );
  };
    ...
    <>
        <Wrap>
            ...
            {bannerImgList.map((character, index) => (
                    <EventSlide key={index} character={character} isActive={index === slideIndex} />
            ))}
            {/* 오른쪽 화살표 버튼 */}
...
const ArrowBtn = styled.button`
    ...
    outline:none;
    cursor:pointer;
    left: ${props => props.direction === "prev" ? '20%' : 'unset'}; // 양쪽 끝으로부터 padding 주기
...
const StyledArrowImg = styled(ArrowImg)`
  width: 40px;
  height: 40px;
  transform: ${props => props.direction === "prev" ? 'rotate(90deg)' : 'rotate(-90deg)'}; // '이전'버튼이면 90도 돌리고 아니면 -90도 돌리기
`;