...
import ItemSlide from '../mainPage/ItemSlideWrap';
...
function ItemSlider() {
  ...
  const moveToSlide = (direction) => {
    setSlideIndex(prev => 
      direction === "prev"
        ? prev === 0 ? Math.floor(itemList.length / 4) - 1 : prev - 1 
        : prev === Math.floor(itemList.length / 4) - 1 ? 0 : prev + 1 
    );
  };
  ...
  return (
    <>
        <Wrap>
            ...
            {Array(Math.ceil(itemList.length / 4)).fill().map((_, i) => (
                <ItemSlide key={i} items={itemList.slice(i * 4, (i + 1) * 4)} isActive={i === slideIndex} />
            ))}
...
const ArrowBtn = styled.button`
    ...
    outline:none;
    cursor:pointer;
    left: ${props => props.direction === "prev" ? '8%' : 'unset'}; // 양쪽 끝으로부터 padding 주기
