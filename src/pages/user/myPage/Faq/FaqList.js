...
import FaqFoldItem from './FaqFoldItem';
import { FaqDataList, FaqTableTitle } from '../../../../assets/datas/UserFAQData';
...
const FaqList = () => {
    const [FoldCheck, setFoldCheck] = useState(Array(FaqDataList.length).fill(false));
    const [selectedId, setSelectedId] = useState(1); // 선택된 id를 관리하는 상태

    // 선택된 id에 따라 FaqDataList 필터링
    const filteredFaqDataList = FaqDataList.filter((faqData) => faqData.subjectId === selectedId);
            ...
            <Table>
                <Header>
                    {FaqTableTitle.map((item, index) => (
                        <Col key={index} width={colWidths[index]}className='cm-SRegular18' onClick={() => setSelectedId(item.id)}>{item.subject}</Col>
                    ))}             
                </Header>
                ...
                <Main>
                    {filteredFaqDataList.map((item, index) => (
                        <>
                            <Row key={index} onClick={() => handleItemClick(index)}> {/*세로 정렬 */}
                                <Item>
